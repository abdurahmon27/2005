...
export async function GET() {
  try {
    ...
    return NextResponse.json({ 
      ...
      averageViewsPerVideo: Math.floor(parseInt(viewCount, 10) / parseInt(videoCount, 10)),
      subscriberCount: parseInt(subscriberCount, 10)
    })
  } catch (error) {
    return NextResponse.json({ error: 'Failed to fetch video count data' }, { status: 500 })
  }
}
