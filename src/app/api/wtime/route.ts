...
export async function GET() {
  try {
    ...
    // You might want to integrate with YouTube Analytics API for more accurate watch time data
    const estimatedWatchTimeMinutes = Math.floor(parseInt(viewCount, 10) * 0.5 * 8) // Assuming 8 min average video length, 50% retention
    ...
    return NextResponse.json({ error: 'Failed to fetch watch time data' }, { status: 500 })
  }
}
