...
export interface TemporaryPageData {
  id: string;
  title: string;
  route: string;
  publish_date: string;
...
/**
 * Fetch the RTS (temporary) page regardless of its published status
 * @returns Promise<TemporaryPageData | null>
 */
export async function getRtsPage(): Promise<TemporaryPageData | null> {
  try {
    // Get all pages without published filter (published = undefined)
    const allPages = await getPages(undefined);
    
    // Find the RTS page
    const rtsPage = allPages.find(
      (post) => post.route === "rts" || post.title.toLowerCase().includes("rts")
    );
    ...
    // Get the content for the RTS page
    const content = await getPageContent(rtsPage.id);
    ...
    console.error("Error fetching RTT page:", error);
    return null;
  }
}
